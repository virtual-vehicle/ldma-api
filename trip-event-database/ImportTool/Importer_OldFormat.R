...
source("getDBConnectionRemote.R")
...
uuids <-  replicate(n_trips, UUIDgenerate())

if (length(unique(uuids)) != n_trips) {

  stop("uuids are not unique!") ## sometime that happens; I do not know when or why
