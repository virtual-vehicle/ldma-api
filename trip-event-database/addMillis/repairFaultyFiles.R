...
for (i in seq_len(nrow(infos))) {
  ...
  if (infos$newFormat[i]) {
    ...
    addMillisToOldFormat(file.path(infos$folder[i], "accelerations.csv"))
    addMillisToOldFormat(file.path(infos$folder[i], "gyroscopes.csv"))
    addMillisToOldFormat(file.path(infos$folder[i], "obddata.csv"))
    addMillisToOldFormat(file.path(infos$folder[i], "positions.csv"))
  }
