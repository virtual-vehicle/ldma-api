...
source("docker/analytics_processor/helpers/event_creation_helpers.R")
source("helpers/db_batch_insert.R")
source("helpers/sql_str_quote.R")

extract_events_processor <- function(con, id) {
  ...
  event_counts <- tibble(
    trip_id = sql_str_quote(trip$trip_id),
    n_brake = brake_acc_events %>% filter(event_type == "'BRAKE'") %>% tally() %>% pull(n),
    n_acceleration =  brake_acc_events %>% filter(event_type == "'ACCELERATION'") %>% tally() %>% pull(n),
    n_standstill = NA_integer_, ##filled later when events are calulated
    n_pothole = nrow(pothole_events)
  )
  ...
  tryCatch({
    ...
    db_batch_insert(con, "trip_data.event_t", brake_acc_events, 10000)
    ...
    db_batch_insert(con, "trip_data.event_t", pothole_events, 10000)
    
    standstill_events <- getStandstillEvents(dat, trip, brake_events) %>% quoteEvents()
    db_batch_insert(con, "trip_data.event_t", standstill_events, 10000)
    
    ## inster event counts
    
    event_counts$n_standstill <- nrow(standstill_events)
      
    db_batch_insert(con, "trip_data.event_count_t", event_counts, 10000)
